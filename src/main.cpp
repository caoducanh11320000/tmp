...
int main(int argc, char** argv) {
    ...
    else if (string(argv[1]) == "-d" && string(argv[2])== "-i")
    {
        ...
        {
            std::cout << "Bounding box: " << x.results[j].ClassID<<" ,P= " << x.results[j].Confidence<<" ,bounding box: " << x.results[j].bbox.x<<" " << x.results[j].bbox.y<<" " << x.results[j].bbox.hight<<" " << x.results[j].bbox.weight << std::endl;
        }
        }
        // Realease
        test1.trt_release();
